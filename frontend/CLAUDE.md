...
| `WelcomeDisclaimer.tsx`, `welcomeState.ts` | The "Antes de empezar" notice (spec 009) at the top of the home, only when the route's `state` is `WELCOME_STATE` — passed by `useSignupForm` and `CompleteGoogleSignupPage` when they navigate to `/home` after creating the account. "Entendido" clears the state (`replace`), so neither a reload nor a later login shows it. Any new way of creating an account must navigate with `{ state: WELCOME_STATE }` |
