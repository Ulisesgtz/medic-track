...
describe('createConsultation', () => {
  ...
  it('returns the created consultation on success', async () => {
    ...
    expect(await createConsultation('child-1', { doctorName: 'Dra. López', consultDate: '2026-01-15', photoBase64: 'Zm9v', medications: [medicationPayload] })).toEqual(created)
  })

  it('throws child_not_found on 404', async () => {
    vi.mocked(fetch).mockResolvedValueOnce({ ok: false, status: 404, json: async () => ({}) } as Response)

    const err = await createConsultation('missing', { doctorName: 'X', consultDate: '2026-01-15', photoBase64: 'x', medications: [medicationPayload] }).catch((e) => e)
    expect(err.kind).toBe('child_not_found')
  })
  ...
  it('throws validation_error on 400', async () => {
    ...
    const err = await createConsultation('child-1', { doctorName: '', consultDate: '2026-01-15', photoBase64: 'x', medications: [] }).catch((e) => e)
  ...
  it('throws unknown for an unexpected status code', async () => {
    ...
    const err = await createConsultation('child-1', { doctorName: 'X', consultDate: '2026-01-15', photoBase64: 'x', medications: [medicationPayload] }).catch((e) => e)
    expect(err.kind).toBe('unknown')
  })
})
