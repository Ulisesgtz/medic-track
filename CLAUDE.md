...
- `specs/007-homologar-pantallas-a-mocks/` — **top-priority backlog item, in progress**: make every screen match the mocks the user delivered exactly (see the per-screen table in its spec.md, including the mocks still missing). On desktop `/home` opens the active child's detail (mock 6, "Home con detalle"). Branch `feature/007-homologar-pantallas-a-mocks`.
